...
public final class CSSParser {
    ...
    public static List<Rule> parse(final String css) throws Exception {
        ...
        for (int i = 0; i < css.length(); i++) {
            ...
            if (i < css.length() - 1) {
                ...
                final char nextC = css.charAt(i + 1);
    ...
    private void parseValue(final Character c) throws IncorrectFormatException {
        ...
        if (Chars.SEMI_COLON.equals(c)) {
            ...
            final PropertyValue pv = new PropertyValue(propertyName.trim(), valueName.trim());
            values.add(pv);
            ...
            throw new IncorrectFormatException(ErrorCode.FOUND_COLON_WHILE_READING_VALUE, "The value '"
                    + valueName.trim() + "' for property '" + propertyName.trim() + "' in the selector '"
                    + selectorName.trim() + "' had a ':' character.");

        } else if (Chars.BRACKET_END.equals(c)) {

            throw new IncorrectFormatException(ErrorCode.FOUND_END_BRACKET_BEFORE_SEMICOLON, "The value '"
                    + valueName.trim() + "' for property '" + propertyName.trim() + "' in the selector '"
                    + selectorName.trim() + "' should end with an ';', not with '}'.");
    ...
    private void parsePropertyName(final List<Rule> rules, final Character c) throws IncorrectFormatException {
        ...
        if (Chars.COLON.equals(c)) {
            ...
            throw new IncorrectFormatException(ErrorCode.FOUND_SEMICOLON_WHEN_READING_PROPERTY_NAME,
                    "Unexpected character '" + c + "' for property '" + propertyName.trim() + "' in the selector '"
                            + selectorName.trim() + "' should end with an ';', not with '}'.");
            ...
            for (final String s : selectorNames) {
                final Selector selector = new Selector(s.trim());
                rule.addSelector(selector);
            }
            selectorNames.clear();

            final Selector selector = new Selector(selectorName.trim());
            selectorName = "";
            rule.addSelector(selector);

            // Add the property values
            for (final PropertyValue pv : values) {
    ...
    private void parseSelector(final Character c) throws IncorrectFormatException {
        ...
        if (Chars.BRACKET_BEG.equals(c)) {
            ...
            if (selectorName.trim().isEmpty()) {
                throw new IncorrectFormatException(ErrorCode.FOUND_COLON_WHEN_READING_SELECTOR_NAME,
