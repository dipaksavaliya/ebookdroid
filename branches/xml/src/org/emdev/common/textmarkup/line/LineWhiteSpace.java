...
public class LineWhiteSpace extends AbstractLineElement {
    ...
    public static void addToLines(MarkupStream stream, LineStream lines) throws IOException {
        final int position = stream.in.position() - 1;
        final float width = stream.in.readFloat();
        final int height = stream.in.readInt();

        AbstractLineElement.addToLines(stream, MarkupTag.LineWhiteSpace, position, null, width, height, lines);
    }
}
