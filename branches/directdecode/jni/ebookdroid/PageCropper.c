...
float getLeftBound(uint8_t* src, int width, int height, float avgLum);
float getLeftColumnBound(uint8_t* src, int width, int height, float avgLum, float x, float y);
float getTopBound(uint8_t* src, int width, int height, float avgLum);
float getRightBound(uint8_t* src, int width, int height, float avgLum);
float getRightColumnBound(uint8_t* src, int width, int height, float avgLum, float x, float y);
float getBottomBound(uint8_t* src, int width, int height, float avgLum);

JNIEXPORT jobject JNICALL
Java_org_ebookdroid_core_crop_PageCropper_nativeGetCropBounds(JNIEnv* env, jclass clz, jobject pixels, jint width,
		jint height, jfloat left, jfloat top, jfloat right, jfloat bottom) {
...
int isRectWhite(uint8_t* src, int width, int height, int sub_x, int sub_y, int sub_w, int sub_h, int avgLum) {
	...
	for (y = 0; y < sub_h; y++) {
		for (x = 0; x < sub_w; x++) {
			...
			int lum = (MIN(src[i+2],MIN(src[i + 1],src[i])) + MAX(src[i+2],MAX(src[i + 1],src[i]))) / 2;
	...
	return ((float) count / (width * height)) < WHITE_THRESHOLD;
}
