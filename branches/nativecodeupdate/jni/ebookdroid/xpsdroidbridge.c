...
//    return -1;
    ...
    fz_try(doc->ctx)
    {
	page = fz_load_page(doc->xpsdoc, pageNumber - 1);
	bounds = fz_bound_page(doc->xpsdoc, page);
    }
    fz_catch(doc->ctx)
    {
        return -1;
...
{
    renderdocument_t *doc = (renderdocument_t*) (long) handle;
    return (fz_count_pages(doc->xpsdoc));
}
...
Java_org_ebookdroid_droids_xps_codec_XpsPage_open(JNIEnv *env, jclass clazz, jlong dochandle, jint pageno)
{
    ...
    fz_try(doc->ctx)
    {
	page->page = fz_load_page(doc->xpsdoc, pageno - 1);
    }
    fz_catch(doc->ctx)
    {
	free(page);
...
//New draw page
    ...
    fz_run_page(doc->xpsdoc, page->page, dev, fz_identity, NULL);
    fz_free_device(dev);
//
...
Java_org_ebookdroid_droids_xps_codec_XpsPage_free(JNIEnv *env, jclass clazz, jlong docHandle, jlong handle)
{
    ...
    {
        if (page->page)
            fz_free_page(doc->xpsdoc, page->page);
...
Java_org_ebookdroid_droids_xps_codec_XpsPage_getBounds(JNIEnv *env, jclass clazz, jlong dochandle, jlong handle, jfloatArray bounds)
{
    ...
    if (!bbox)
        return;
    fz_rect page_bounds = fz_bound_page(doc->xpsdoc, page->page);
...
Java_org_ebookdroid_droids_xps_codec_XpsOutline_open(JNIEnv *env, jclass clazz, jlong dochandle)
{
    ...
    if (!doc->outline)
        doc->outline = fz_load_outline(doc->xpsdoc);
